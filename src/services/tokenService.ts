...
import { generateInitialPrice, calculateBuyCost, calculateSellReturn, floor4, MIN_TRADE_AMOUNT } from "./pricingService";
 ...
 */
export async function getTradeQuote(tokenId: number, side: "buy" | "sell", rawAmount: number) {
...
export async function boostToken(tokenId: number, userId: number, amount: number) {
  ...
  try {
    ...
    // XATOLIK TUZATILDI: avval yangi narx sof formula bo'yicha (tebranish
    // "drift"ini hisobga olmasdan) qayta hisoblanardi - natijada narx
    // tebranish bilan ko'tarilgan bo'lsa, boost narxni OSHIRISH o'rniga
    // TUSHIRIB yuborishi mumkin edi. Endi joriy narx aynan boost foiziga oshadi.
    const newCurrentPrice = currentPrice * (1 + boostPct);
